...
async function syncGrus(data: string): Promise<{ novos: number; total: number }> {
  ...
  const filtros = `ugArrecadadora=${SISGRU_UG}&dtEmissao=${data}-${data}`
...
async function syncPagamentos(data: string): Promise<{ novos: number; total: number }> {
  const jwt = gerarJwtSisgru()
  const filtros = `ug=${SISGRU_UG}&dataAlteracaoSituacaoPagTesouro=${data}-${data}`
  ...
  for (const pag of pagamentos) {
    ...
    const foiInserido = result.rowCount > 0
    if (foiInserido) {
      novos++

      // Acumular créditos de reprografia apenas para pagamentos novos com servico_id = 16279
      if (pag.servicoId === 16279) {
        await query(
...
/**
 * Sincroniza GRUs e Pagamentos do dia.
 * Falha em um endpoint não cancela o outro.
 * @param data - Data no formato DD/MM/YYYY (padrão: hoje)
 */
async function syncDia(data?: string): Promise<void> {
  const dataAlvo = data ?? formatarData(new Date())

  // Sincronizar GRUs
  try {
    console.log(`[sisgruSync] Iniciando sync de GRUs para ${dataAlvo}`)
    const resultado = await syncGrus(dataAlvo)
    console.log(
  ...
  // Sincronizar Pagamentos (independente do resultado dos GRUs)
  try {
    console.log(`[sisgruSync] Iniciando sync de Pagamentos para ${dataAlvo}`)
    const resultado = await syncPagamentos(dataAlvo)
    console.log(
