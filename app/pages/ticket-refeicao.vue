<template>
  <div>
    ...
    <TicketModalEstorno ref="modalEstornoRef" @estornado="onEstornoRealizado" />
  </div>
</template>
...
function logConfirmar(err: unknown): void {} // Dummy replace pra remover o bloco

function abrirModalEstorno(transacao: Transacao): void {
  modalEstornoRef.value?.abrir(transacao)
}

async function onEstornoRealizado(): Promise<void> {
  await carregarTransacoes()
  if (resumo.value?.cpf) {
    await buscar()
  }
